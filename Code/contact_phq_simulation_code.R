...
#library(groundhog)
#groundhog.library(dplyr, "2025-11-25")
#groundhog.library(faux, "2025-11-25")
#groundhog.library(summarytools, "2025-11-25")
#groundhog.library(missMethods, "2025-11-25")
...
# Age 
age <- rnorm(n = 500, mean = 27.5, sd = 0.45)

#sex (0 = female, 1 = male) It assumes 60% Male
sex <- rbinom(N, 1, 0.6)

# race (0 = BIPOC, 1 = White) It assumes 40% White
race <- rbinom(N, 1, 0.4) 

#Immigrant (0= Immigrant, 1 = non-immigrant)
immigrant <- rbinom(N, 1, 0.5)
...
sim_data <- data.frame(
  ...
  immigrant = factor(immigrant, levels=c(0,1), labels=c("immigrant","non-immigrant")),
...
sim_data <- missMethods::delete_MCAR(sim_data, .03, "phq_5")
sim_data <- missMethods::delete_MCAR(sim_data, .03, "phq_6")
sim_data <- missMethods::delete_MCAR(sim_data, .02, "phq_7")
sim_data <- missMethods::delete_MCAR(sim_data, .01, "phq_8")
sim_data <- missMethods::delete_MCAR(sim_data, .01, "phq_9")
sim_data <- missMethods::delete_MCAR(sim_data, .01, "pcon")

write.csv(sim_data, "contact_phq_simulated_2025-12-08.csv", row.names = FALSE)
